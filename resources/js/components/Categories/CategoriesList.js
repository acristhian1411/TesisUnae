    ...
    class CategoriesList extends Component {
      constructor () {
        ...
        this.getObject = async(perPage, page, orderBy=this.state.orderBy, order=this.state.order) =>{
           let res = await axios.get(`/api/categories?sort_by=${orderBy}&order=${order}&per_page=${perPage}&page=${page}`);
           let data = await res.data;
           this.setState({categories: data, paginator:data, filteredcategories: data.data,  open:false, page_lenght: res.data.last_page, page: res.data.current_page});
      }
...
// funciones para abrir Dialog
        this.handleClickOpen = (data) =>{
                this.setState({open: true, category: data});
              }
...
//fin constructor
    ...
    search (event) {
      ...
      if (this.state.time_out==false) {
        ...
        setTimeout(()=>{
          if(e.target.value===''){
            this.setState({
              ...
              paginator:this.state.categories,
              page_lenght: this.state.categories.last_page,
              page: 1,
              time_out:false
            });
          }
          else {
            axios.get(`/api/categories?cat_descript=${e.target.value}`).then(res=>{
              this.setState({
        ...
        deleteObject() {
            axios.delete(`api/categories/${this.state.cat_id}`).then((res) => {
      ...
      render () {
        ...
        var showSnack;
        const {snack_open, message_success, filteredcategories, open, category, orderBy, order} = this.state;
       if (this.state.new) {
        ...
        showModal = <MidModal >
         ...
         // showModal = <ModalForm edit={true} onHandleSubmit={this.updateState} category={this.state.category}/>
         showModal = <MidModal >
                       {{onHandleSubmit: this.updateState,
                         form: <CategoriesForm />,
                         props_form: {edit: true, category:this.state.category, onSuccess:this.openSnack}
                       }}
                     </ MidModal>
       }
       if(open) {
         showDialogDestroy = <DialogDestroy index={category.data.cat_descript} onClose={this.handleClose} onHandleAgree={this.deleteObject}/>;
       }
       if (snack_open){
        ...
        return (
         <div className='card-body'>
                             ...
                             <h2 align='center'>Categorias<TextField id="outlined-search" label="Buscar" type="search" variant="outlined" onChange={this.search} /></h2>
                             <hr />
                             <TableContainer component={Paper}>
                           <Table aria-label="simple table" option={{search: true}}>
                             <TableHead>
                               <TableRow>
                                 <TableCell sortDirection={orderBy === 'cat_descript' ? order : false}>
                                   <TableSortLabel
                                     active={orderBy === 'cat_descript'}
                                     direction={orderBy === 'cat_descript' ? order : 'asc'}
                                     onClick={()=>{this.createSortHandler('cat_descript')}}
                                   >
                                     <h3>Descripcion</h3>
                                   </TableSortLabel>
                                 </TableCell>
      ...
      return(
        ...
        <TableRow>
          ...
          return this.state.filteredcategories.map((data)=>{
            return(
          <TableBody key={data.cat_id}>
              <TableRow>
                <TableCell>{data.cat_descript}</TableCell>
                ...
                <Tooltip title='Eliminar'>
                  <Button variant="outlined" color="secondary" onClick={()=>{this.handleClickOpen({data: data}), this.setState({cat_id:data.cat_id})}}>
                    <DeleteIcon />
                  </Button>
                  </Tooltip>
                </TableCell>
