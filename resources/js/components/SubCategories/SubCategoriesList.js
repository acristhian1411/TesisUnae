    ...
    class SubsubcategoriesList extends Component {
      constructor () {
        super()
        this.state = {
          snack_open: false,
        ...
        this.getObject = async(perPage, page, orderBy=this.state.orderBy, order=this.state.order) =>{
           let res = await axios.get(`/api/subcategories?sort_by=${orderBy}&order=${order}&per_page=${perPage}&page=${page}`);
           let data = await res.data;
           this.setState({subcategories: data, paginator:data, filteredsubcategories: data.data,  open:false, page_lenght: res.data.last_page, page: res.data.current_page});
      }
...
// funciones para abrir Dialog
        this.handleClickOpen = (data) =>{
                this.setState({open: true, subcategory: data});
              }
...
//fin constructor
    ...
    search (event) {
      ...
      if (this.state.time_out==false) {
        ...
        setTimeout(()=>{
          if(e.target.value===''){
            this.setState({
              filteredsubcategories:this.state.subcategories.data,
              paginator:this.state.subcategories,
              page_lenght: this.state.subcategories.last_page,
              page: 1,
              time_out:false
            });
          }
        ...
        deleteObject() {
            axios.delete(`api/subcategories/${this.state.sub_cat_id}`).then((res) => {
      ...
      render () {
        ...
        var showSnack;
        const {snack_open, message_success, open, filteredsubcategories, subcategory, orderBy, order} = this.state;
       if (this.state.new) {
       ...
       if(open) {
         showDialogDestroy = <DialogDestroy index={subcategory.data.sub_cat_descrip} onClose={this.handleClose} onHandleAgree={this.deleteObject}/>;
       }
       if (snack_open){
        ...
        return (
         <div className='card-body'>
                             ...
                             <h2 align='center'>SubCategorias<TextField id="outlined-search" label="Buscar" type="search" variant="outlined" onChange={this.search} /></h2>
                             <hr />
                             <TableContainer component={Paper}>
                           <Table aria-label="simple table" option={{search: true}}>
                             <TableHead>
                               <TableRow>
                                 <TableCell sortDirection={orderBy === 'sub_cat_descrip' ? order : false}>
                                   <TableSortLabel
                                     active={orderBy === 'sub_cat_descrip'}
                                     direction={orderBy === 'sub_cat_descrip' ? order : 'asc'}
                                     onClick={()=>{this.createSortHandler('sub_cat_descrip')}}
                                   >
                                     <h3>Descripcion</h3>
                                   </TableSortLabel>
                                 </TableCell>
      ...
      selectValue(event){
        ...
        renderList(){
          if(this.state.filteredsubcategories.length === 0){
      return(
        <TableBody>
        <TableRow>
          <TableCell>{' '}</TableCell>
          ...
          return this.state.filteredsubcategories.map((data)=>{
            return(
          <TableBody key={data.sub_cat_id}>
              <TableRow>
                <TableCell>{data.sub_cat_descrip}</TableCell>
                ...
                <Tooltip title='Eliminar'>
                  <Button variant="outlined" color="secondary" onClick={()=>{this.handleClickOpen({data: data}), this.setState({sub_cat_id:data.sub_cat_id})}}>
                    <DeleteIcon />
                  </Button>
                  </Tooltip>
                </TableCell>
    ...
    export default SubsubcategoriesList
