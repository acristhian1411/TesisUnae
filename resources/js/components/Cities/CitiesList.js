    ...
    class CitiesList extends Component {
      constructor () {
        ...
        this.getObject = async(perPage, page, orderBy=this.state.orderBy, order=this.state.order) =>{
           let res = await axios.get(`/api/cities?sort_by=${orderBy}&order=${order}&per_page=${perPage}&page=${page}`);
           let data = await res.data;
           this.setState({cities: data, paginator:data, filteredcities: data.data,  open:false, page_lenght: res.data.last_page, page: res.data.current_page});
      }
    ...
    search (event) {
      ...
      if (this.state.time_out==false) {
        ...
        setTimeout(()=>{
          if(e.target.value===''){
            this.setState({
              filteredcities:this.state.cities.data,
              paginator:this.state.cities,
              page_lenght: this.state.cities.last_page,
              page: 1,
              time_out:false
            });
          }
          else {
            axios.get(`/api/cities/search/${e.target.value}`).then(res=>{
              this.setState({
      ...
      render () {
        ...
        var showSnack;
        const {snack_open, message_success, filteredcities, open, city, orderBy, order} = this.state;
       if (this.state.new) {
        ...
        showModal = <MidModal >
         ...
         showModal = <MidModal >
                       {{onHandleSubmit: this.updateState,
                         form: <CityForm />,
                         props_form: {edit: true, city:this.state.city, onSuccess:this.openSnack}
                       }}
                     </ MidModal>
       }
       if(open) {
         showDialogDestroy = <DialogDestroy index={city.data.city_descrip} onClose={this.handleClose} onHandleAgree={this.deleteObject}/>;
       }
       if (snack_open){
        ...
        return (
         <div className='card-body'>
                           ...
                           <Table aria-label="simple table" option={{search: true}}>
                             <TableHead>
                               <TableRow>
                                 <TableCell sortDirection={orderBy === 'city_descrip' ? order : false}>
                                   <TableSortLabel
                                     active={orderBy === 'city_descrip'}
                                     direction={orderBy === 'city_descrip' ? order : 'asc'}
                                     onClick={()=>{this.createSortHandler('city_descrip')}}
                                   >
                                     <h3>Descripcion</h3>
                                   </TableSortLabel>
                                 </TableCell>
      ...
      return(
        ...
        <TableRow>
          ...
          <TableBody key={data.city_id}>
              <TableRow>
                <TableCell>{data.city_descrip}</TableCell>
                ...
                <Tooltip title='Eliminar'>
                  <Button variant="outlined" color="secondary" onClick={()=>{this.handleClickOpen({data: data}), this.setState({city_id:data.city_id})}}>
                    <DeleteIcon />
                  </Button>
                  </Tooltip>
                </TableCell>
